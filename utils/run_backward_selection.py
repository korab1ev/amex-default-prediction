...
def run_backward_selection(df_train,
    ...
    tuned_max_depth, tuned_estimators = 6, 350
    ...
    while True:
        ...
        for fold, (tr_idx, va_idx) in enumerate(
            ...
            print(f"Fold {fold} Amex = {sc:.5f}")
            fold_scores.append(sc)
            fold_test_raw.append(model.predict_proba(df_test[feats])[:,1])
    ...
    # -------- final model ------------------------------------------
    final_model = LGBMClassifier(**params).fit(
        df_train[num+cat], df_train[target_col], categorical_feature=cat)

    return (pd.DataFrame(history), list_imp_dfs,
            {'num': num, 'cat': cat}, final_model, test_preds_df)
